...
async function getMenu(id_restaurante){
 ...
 document.getElementById('restaurantmorada').innerHTML = "Morada: " + suggestedrestaurants[0].local_morada;
  ...
  function createreservaHTML(reserva_mesa){
    ...
    return "<div id='reportitem' style='border: 2px;  border-color: black; display: inline-block; background-color: rgb(236, 236, 236); width: 70%; height: 23%; position: relative;'><h3 id='utilizadorname' style='margin-left: 1.6%; font-size: 27px;'>" + reserva_mesa.utilizador_name + "</h3><h3 id='numeromesaname' style='margin-left: 1.6%; font-size: 16px; margin-top: -2%;'>" + reserva_mesa.mesa_number + "</h3><h3 id='tipoetamanhoname' style='margin-left: 1.6%; margin-top: -1.6%;'>Tamanho: <i>" + reserva_mesa.mesa_size + "</i> | Tipo: <i>" + reserva_mesa.mesa_type_name + "</i></h3><h3 id='dataname' style='margin-left: 1.6%;  margin-top: -1.2%;'>Data: " + reserva_mesa.date_marcada_reservation + "</h3><button style='margin-left:73%; margin-top: -10%; position: absolute;' id='button9' onclick='removerReservaMesa(" + JSON.stringify(reserva_mesa) + ")'>CANCELAR A RESERVA</button></div>";
